        ...
        <div class="container">
            ...
            if(isset($_POST['submit']) && $_POST['first_number']!="" && $_POST['second_number']!=""){
                ...
                if(!preg_match('/^[1-9][0-9]*$/', $first) || !preg_match('/^[1-9][0-9]*$/', $second)){
                    $msg = "The string must contain only numbers and the leading number should not be a zero";
                }else{
                    $first_rev = array_reverse(str_split($first));
                    $second_rev = array_reverse(str_split($second));
                    
                    //multiply each digit and sum at the corresponding positions
                    for($i=0; $i<count($first_rev); $i++){
                        for($j=0; $j<count($second_rev); $j++){
                            $d[$i+$j] += ($first_rev[$i]-'0') * ($second_rev[$j]-'0');
                        }
                    }
                    ...
                    var_dump($sb);
                    // die();
                    $msg = "The product of ".$first." and ". $second." is ".strrev($sb);
